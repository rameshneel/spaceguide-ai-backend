...
export const getSubscriptionUsage = asyncHandler(async (req, res) => {
  ...
  const isUnlimited = (limit) => limit && limit >= UNLIMITED_THRESHOLD;

  // Calculate usage percentages (handle unlimited plans)
  const usageStats = {
    aiTextWriter: {
      wordsUsed: wordsUsedToday,
      wordsLimit: limits.aiTextWriter?.wordsPerDay || 0,
      wordsPercentage: isUnlimited(limits.aiTextWriter?.wordsPerDay)
        ? 0 // Unlimited = 0% (no percentage for unlimited)
        : limits.aiTextWriter?.wordsPerDay
      ...
      isUnlimited: isUnlimited(limits.aiTextWriter?.wordsPerDay),
    },
    aiImageGenerator: {
      imagesUsed: imagesUsedToday,
      imagesLimit: limits.aiImageGenerator?.imagesPerDay || 0,
      imagesPercentage: isUnlimited(limits.aiImageGenerator?.imagesPerDay)
        ? 0 // Unlimited = 0% (no percentage for unlimited)
        : limits.aiImageGenerator?.imagesPerDay
        ? Math.round(
            (imagesUsedToday / limits.aiImageGenerator.imagesPerDay) * 100
          )
        : 0,
      isUnlimited: isUnlimited(limits.aiImageGenerator?.imagesPerDay),
    },
    aiSearch: {
      searchesUsed: 0, // This would need to be tracked separately
      searchesLimit: limits.aiSearch?.searchesPerDay || 0,
      searchesPercentage: isUnlimited(limits.aiSearch?.searchesPerDay)
        ? 0 // Unlimited = 0% (no percentage for unlimited)
        : limits.aiSearch?.searchesPerDay
        ? Math.round((0 / limits.aiSearch.searchesPerDay) * 100)
        : 0,
      isUnlimited: isUnlimited(limits.aiSearch?.searchesPerDay),
    },
    aiChatbot: {
      chatbotsUsed: chatbotsUsed,
      chatbotsLimit: limits.aiChatbot?.chatbotsPerAccount || 0,
      chatbotsPercentage: isUnlimited(limits.aiChatbot?.chatbotsPerAccount)
        ? 0 // Unlimited = 0% (no percentage for unlimited)
        : limits.aiChatbot?.chatbotsPerAccount
        ? Math.round((chatbotsUsed / limits.aiChatbot.chatbotsPerAccount) * 100)
        : 0,
      isUnlimited: isUnlimited(limits.aiChatbot?.chatbotsPerAccount),
    },
  };
