...
const getDefaultProvider = () => {
  ...
  if (process.env.NODE_ENV === "production") {
    return "openai"; // DALL·E 3 for production
  }

  // Development: Try free providers in order (Pollinations doesn't need API key!)
  // Always available, no API key needed
  if (process.env.USE_POLLINATIONS !== "false") {
    return "pollinations"; // Completely free, no API key!
  }
  ...
  return "pollinations"; // Default to Pollinations (free, no API key needed!)
};
...
export class AIImageGeneratorService {
  ...
  async generateImageWithDALLE(prompt, options = {}) {
    ...
    try {
      ...
      // Fallback to mock image for development
      if (process.env.NODE_ENV !== "production") {
